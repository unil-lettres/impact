<?php

return [

    'states' => 'States',
    'private' => 'private',
    'private_description' => 'The card is visible and editable only by the holder(s).',
    'open' => 'open',
    'open_description' => 'The card is visible to the holder(s) and manager(s), but not to the members of this space.',
    'public' => 'public',
    'public_description' => 'The card is visible to all users of this space.',
    'archived' => 'archived',
    'archived_description' => 'The card is no longer editable by the holder(s).',
    'new_state' => 'new state',
    'permission0' => 'No permission defined',
    'permission1' => 'Visible to holder(s) and manager(s), editable by manager(s) only',
    'permission2' => 'Visible and editable by the holder(s) only',
    'permission3' => 'Visible and editable by the holder(s) and manager(s)',
    'permission4' => 'Visible to all, editable by the holder(s) and manager(s)',
    'permission5' => 'Visible to all, editable by manager(s) only',
    'permission6' => 'Visible and editable by manager(s) only',
    'permissions' => 'Permissions',
    'add' => 'Add state',
    'update' => 'Update state',
    'delete' => 'Delete state',
    'general' => 'General information',
    'name' => 'Name',
    'description' => 'Description',
    'teachers_only' => 'Managers only',
    'teachers_only_help' => 'Only managers can choose this state.',
    'box1' => 'Box 1',
    'box2' => 'Box 2',
    'box3' => 'Box 3',
    'box4' => 'Box 4',
    'box5' => 'Box 5',
    'action' => 'Action',
    'action_none' => 'Do nothing',
    'action_none_content' => 'No action is defined for this state',
    'action_email' => 'Email to managers',
    'email_subject' => 'Subject',
    'email_message' => 'Message',
    'email_message_help' => 'You can use variables that will be replaced when the email is sent with parameters of the relevant card. The {{title}} tag will be replaced by the title of the card. The {{url}} tag will be replaced by the URL of the card',
    'email_subject_open' => 'Card made visible to managers',
    'email_message_open' => 'The {{title}} card is now visible to the managers. It can be viewed with the following link: {{url}}.',
    'email_subject_public' => 'Card made visible to all',
    'email_message_public' => 'The {{title}} card is now visible to all. It can be viewed with the following link: {{url}}.',

];
