            ...
            <div class="col-md-8">
                @if(Helpers::areCardSettingsEditable($card))
                    ...
                    <div id="rct-single-state-select"
                         reference="state"
                         data='{{ json_encode(['options' => $states, 'default' => $card->state, 'clearable' => false, 'message' => Auth::user()->isManager($card->course) ? null : ['content' => trans('cards.state_cannot_cancel'), 'type' => 'text-danger']]) }}'
                    ></div>
                @else
